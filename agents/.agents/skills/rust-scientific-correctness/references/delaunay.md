...
- Treat every local move, mutation, and repair as a two-outcome transaction. Establish that the pre-move state satisfies the claimed validation layers; on success, validate the committed state through those layers; on failure, restore the complete pre-move owner state and revalidate it. Counts or a partial topology snapshot alone do not prove failure atomicity. Bounded exhaustion remains typed non-convergence rather than silent partial success.
...
- Check local moves through link and f-vector invariants. Exercise failures after meaningful mutation stages and compare canonical storage plus derived indexes, caches, identity, generation, and provenance against the pre-move state before revalidating it. When a concrete inverse exists, require the documented round-trip identity, including vertex and top-dimensional incidence semantics rather than only recovered counts.
