...
`review-graph` uses this matrix as repository-layer semantic evidence and the
classifier in `review_graph_plan.py` as a conservative floor. Return one
disposition for every repository-layer `review-graph` entry in the
machine-readable routing catalog. Silence is not a skip. Resolve any
disagreement with classifier signals explicitly, preserve every shared owner,
select independent review for a concrete change target, and always select
